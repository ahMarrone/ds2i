...
QUERY_TYPES = ["maxscore", "maxscore_dyn", "maxscore_smart_dyn"]
...
print "Q_DATA \t" + "\t".join(QUERY_TYPES) + "\t" + "DELTAS \t MIN\t MAX"
min_counts = [0,0,0]
max_counts = [0,0,0]
for k in range(len(query_metadata[0])):
	...
	print  q_data + "\t" + str(values[0]) + "\t" + str(values[1]) + "\t" + str(values[2]) + "\t" + ";".join(str(x) for x in time_deltas) + "\t" + str(minpos) + "\t" + str(maxpos)
